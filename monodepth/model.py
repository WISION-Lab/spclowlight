...
class PTModel(nn.Module):
    ...
    def forward(self, x):
        ...
        if(self.training):
            #return self.decoder( feats ), [feats[3], feats[4], feats[6], feats[8], feats[12]]
            return self.decoder( feats ), feats[12].unsqueeze(1)
        return self.decoder(feats)


class PTResModel(nn.Module):
    def __init__(self, pretrained_weights=None):
        super(PTResModel, self).__init__()
        self.encoder = networks.ResNetEncoder(BasicBlock, [3, 4, 6, 3])
    ...
    def forward(self, x):
        feats = self.encoder(x)
        if(self.training):
            return self.decoder( feats ), feats[6]
        return self.decoder(feats)
